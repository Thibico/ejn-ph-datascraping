curl 'https://wabi-east-asia-a-primary-api.analysis.windows.net/public/reports/querydata?synchronous=true' \
  ...
  -H 'dnt: 1' \
  -H 'sec-gpc: 1' \
  --data-binary "@test_csv_body_extension2.txt" \
  --compressed | sed 's/"jobIds":\[[^]]*\],//g' | sed 's/"jobId":"[^"]*",//g' | sed 's/"rootActivityId":"[^"]*",//g' | sed 's/"timestamp":"[^"]*",//g' | jq '.' > "./json_files/wastewater_oct15_2nd_ext2.json"